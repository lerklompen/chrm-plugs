...
touch "$WORKDIR/test_writing_to_file" && rm "$WORKDIR/test_writing_to_file" ||
...
if [ ! "$user_image" ]; then
  ...
  # Make the user pick an image to
  # download, or exit.
  choose_image
...
if [ "$choice" = "1" ]; then
  ...
  cp -f $WORKDIR/mount_dir/opt/google/chrome/lib* /opt/google/chrome/ 2>&1 > /dev/null
  # or use 'cp -rf' to include dir 'lib'?
