...
if [ -n "${1:-}" ] && [ "$1" != "--dir" ]; then
  ...
  # That's not really true. You can specify "--dir /desired/path".
  exit 1
fi
...
if [ -n "$2" ]; then
  ...
  if [ ! -d "$WORKDIR" ]; then
    fatal "$WORKDIR does not exist, quitting..."
  fi
  ...
  # find in Chromium OS
  if [ -z "$DISKUTIL" ]; then
    files=($(find . -type f -regextype posix-extended -regex "^\./(chromeos).*\.(bin|zip)"))
  # find on Mac
  else
  ...
  if [ "$files" ]; then
    ...
    for i in "${!files[@]}"; do
      let j=$i+1
      currentfile="${files[$i]:2}"
      #currentfilesize=$((ls -lh "$currentfile") | awk "/$currentfilesize/"'{print $5}')
...
# If the user picks a zip file - unpack the image
if [ "$user_image" ] && [ "${user_image: -4}" = ".zip" ]; then
  ...
  warn "Unpacking the zipfile without checksum or known filesize"
  # Unpack the file
  rm -f "$file"
  if [ "$DECOMPRESS" = "unzip" ]; then
...
if [ "$choice" = "1" ]; then
  ...
  cp $WORKDIR/mount_dir/opt/google/chrome/lib* /opt/google/chrome/ -f
