...
# run 'sudo bash lock.sh' first, then 'sudo bash copysettings.sh' and reboot
# Reconfigure browser:
...
unlock3="sed -i 's/google-chrome --start-fullscreen --window-size=\$Xaxis,\$Yaxis --incognito;/google-chrome --start-fullscreen --window-size=\$Xaxis,\$Yaxis;/' /home/kiosk/.xsession"
...
lock3="sed -i 's/google-chrome --start-fullscreen --window-size=\$Xaxis,\$Yaxis;/google-chrome --start-fullscreen --window-size=\$Xaxis,\$Yaxis --incognito;/' /home/kiosk/.xsession"
echo $lock1 >> lock.sh
echo $lock2 >> lock.sh
echo $lock3 >> lock.sh
