...
if [ -n "$2" ]; then
  ...
  echo "OK, userdirectory set to: $WORKDIR
...
if [ "$user_image" ] && [ "${user_image: -4}" = ".zip" ]; then
  suff=".zip"
  file="${user_image%$suff}"
  zipfilesize=$((ls -l $user_image) | awk "/$files2/"'{print $5}')
  verify_tmp_space $(roundup $(( $zipfilesize + 1509916672 )))
  warn "Unpacking the zipfile without checksum or known filesize"
  # Unpack the file
  rm -f "$file"
  if [ "$DECOMPRESS" = "unzip" ]; then
    if ! unzip "$user_image" "$file"; then
      DEBUG "Can't unpack the zipfile"
      return 1
    fi
  elif [ "$DECOMPRESS" = "zcat" ]; then
    if ! zcat "$user_image" > "$file"; then
      DEBUG "Can't unpack the zipfile"
      return 1
    fi
  fi
